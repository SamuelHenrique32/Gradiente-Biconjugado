...
void _substr(const char* S, const int pos, const int len, char* temp);
void upcase(char* S);
...
/****************************************************************************/
    ...
    char* ThisElement;
    char Title[73], Key[8], Type[4] = {0}, Rhstype[4];
    char Ptrfmt[17], Indfmt[17], Valfmt[21], Rhsfmt[21];
    char line[BUFSIZ];
...
/*************************************************/
    ...
    upcase(fmt);
    tmp = strchr(fmt,'(');
    tmp2 = substr(fmt,tmp - fmt + 1, strchr(fmt,'I') - tmp - 1);
    *perline = atoi(tmp2);
    ...
    tmp = strchr(fmt,'I');
    tmp2 = substr(fmt,tmp - fmt + 1, strchr(fmt,')') - tmp - 1);
...
/*************************************************/
    ...
    tmp = strchr(fmt,'(');
    aux = substr(fmt,tmp - fmt + 1, strchr(fmt,*flag) - tmp - 1);
    *perline = atoi(aux);
    ...
    tmp = strchr(fmt,*flag);
    if ( strchr(fmt,'.') ) {
      aux2 = substr( fmt, strchr(fmt,'.') - fmt + 1, strchr(fmt,')') - strchr(fmt,'.')-1);
      *prec = atoi(aux2);
...
void _substr(const char* S, const int pos, const int len, char* result)
{
   int i;

   if ( pos+len <= strlen(S)) {
      result = (char *)malloc(len+1);
      if ( result == NULL ) IOHBTerminate("Insufficient memory for SubS.");
      for (i=0;i<len;i++) result[i] = S[pos+i];
